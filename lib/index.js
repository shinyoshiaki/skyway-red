...
export class SkyWayRED {
    constructor(options = {}) {
        ...
        this.remoteRedDistance = 0;
        this.redDistance = this.options.redDistance ?? 1;
        this.useAdaptiveRedDistance = this.options.useAdaptiveRedDistance;
    }
    getRTCPeerConnection(connection) {
        connection.open = true;
        const pc = connection.getPeerConnection();
        connection.open = false;
        return pc;
    }
    activateRED(connection) {
        ...
        if (this.useAdaptiveRedDistance) {
            ...
            new Promise(async () => {
                while (true) {
                    ...
                    const remoteInbound = arr.find((a) => a.id.includes("RTCRemoteInboundRtpAudioStream"));
